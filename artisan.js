 ...
 */
async function renderProfile(artisan, craft, error) {
  if (error || !artisan) {
  ...
  if (toggleBtn) {
    ...
    toggleBtn.addEventListener("click", () => {
      ...
      const photosToShow = !isExpanded
        ? workshop_photos
        : workshop_photos.slice(0, 3);

      galleryGrid.innerHTML = photosToShow
        .map(
          (photo, i) =>
            `<div class="gallery-item"><img src="${photo}" alt="Artwork ${
              i + 1
...
async function main() {
  ...
  try {
    ...
    // Now you only need to pass 'data'
    // We assume the renderProfile signature is (artisan, error)
